...
Документ устроен так, что универсальное ядро (§§1–10) применимо к одиночному разработчику, команде из пяти человек и крупной организации одинаково. Корпоративная специфика — регламентированный T3-релиз, 12-месячный план внедрения, конкретный paved-road-стек — вынесена в Appendix A–C; их читают только те, кому это нужно. Источники цифр и кейсов — в Appendix D.
...
- **`CLAUDE.md.template.md`** — **универсальный** baseline: применим в любой команде любого размера, без зависимости от Backstage, AI-Gateway, корпоративных Slack-каналов, обязательного `.github/pull_request_template.md`. Это дефолтный выбор.
- **`CLAUDE.md.template.enterprise.md`** — **корпоративный** baseline: добавляет ссылку на каталог сервисов, обязательный AI-Gateway, эскалацию в корпоративные Slack-каналы, обязательный PR-шаблон с полями «AI-инструменты / что я лично проверил / как тестировал». Берётся, если все эти инфраструктурные слои у вас существуют.
...
**Фаза 0 (0–4 недели).** Корпоративный AI-Gateway с логированием и блокировкой утечек. Allow-list инструментов (например: Claude Code, Claude Cowork, Codex, Cursor). Зафиксировать T0–T3 и label `[ai]` в PR-template. Эталонный `CLAUDE.md.template.md` (универсальный) и `CLAUDE.md.template.enterprise.md` (корпоративный) для paved-road языков.

**Фаза 1 (1–3 месяца).** Каталог сервисов (Backstage или эквивалент) с обязательными метаданными (owner, lifecycle, T-зона, sunset). Scaffolder для предзаполненных сервисов (CI, CODEOWNERS, AGENTS.md, наблюдаемость, secrets-vault). Базовые CI-laws: secret scan, license scan, slopsquatting-check, PR-size 400. AI-Code Guild с явной хартией.
...
**Роли в жизненном цикле кода (расширенно).**

| Роль | Что делает |
|---|---|
| Product Owner | Настройка ролей в Jira; заключение по выпуску; валидация аналитики с CISO; lifecycle-метаданные сервиса. |
| Delivery Manager | Драйвер релиза T3 (см. Appendix A). |
| QA | ИФТ. |
| Ответственный за НТ | Нагрузочное тестирование. |
| ЦКБ | Заключение по выпуску. |
| Проверки безопасности | SAST, lethal-trifecta аудит, threat model, slopsquatting-check; CISO-валидация аналитики. |
| Эксперт сопровождения | Заключение по выпуску; PoB; runbook; on-call; пейджер. |
| AI-Code Guild | Owner библии и шаблонов; пересмотр раз в квартал; чек-лист ревьюера; политики. |
...
**Каталог сервисов.** Backstage с обязательными метаданными (см. Appendix B, Фаза 1).
